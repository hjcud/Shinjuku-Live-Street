...
사람들이 부담 없이 공연을 시작하고 같은 순간을 함께 즐길 수 있도록, 공연 도구와 공유 상호작용, 네트워크 시스템이 서로 연결되어 있습니다.
...
차선을 손으로 배열에 입력하거나 실행할 때마다 문제를 찾는 방식은 월드 규모가 커질수록 유지하기 어렵습니다. 이를 줄이기 위해 Unity Scene에 배치된 차선을 런타임 데이터로 변환하는 베이커와, 차량·센서·네트워크 상태를 Scene View에서 확인하는 디버그 도구를 구성했습니다.
