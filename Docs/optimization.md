...
`10 Hz`는 차량의 주행 판단 주기입니다. 차량 Transform은 권한자와 원격 사용자 모두 매 렌더 프레임 보간하므로 화면이 10 FPS로 갱신되는 구조가 아닙니다. 모델, 머티리얼과 렌더링 설정은 낮추지 않았습니다. GPU 프레임 시간과 화질은 별도 측정하지 않았으므로 이 표에서 다루지 않습니다.
