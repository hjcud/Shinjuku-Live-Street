<p align="center">
  <a href="./optimization.md">한국어</a> · <a href="./optimization.en.md">English</a> · <strong>日本語</strong>
</p>

# 問題と最適化

この文書では機能を並べるのではなく、旧システムで発生していた問題と、現在のコードで変更した内容をまとめています。Profilerで確認した値と、コードから算出した値は分けて記載します。

## 比較条件

- Profiler比較には、初期スナップショットと最新スナップショットの同じ300フレームを使用
- 初期スナップショットは車両ごとの毎フレーム処理、最新スナップショットは中央管理と分散センサーを使う構成
- 稼働車両10台とClientSimのリモートプレイヤー80人を1地点に集めた状態
- フレーム、PlayerLoop、Udon、Physics、GCはUnity ProfilerのCPU測定値
- 1秒あたりの実行回数は60 FPSとして計算
- `RequestSerialization()`の数値は実際に送信されたパケット数ではなく、コードが送信を要求した回数
- モデル関連の数値は現在の`Shinjuku.fbx`と`TEST.unity`の設定値
- 最適化前のモデルは残っていないため、モデルの削減率は記載しない

## 結果の概要

![交通システムの初期スナップショットと最新スナップショットのUnity Profiler比較](./images/traffic-performance-comparison.svg)

| Unity Profiler項目 | 初期スナップショット | 最新スナップショット | 変化 |
| --- | ---: | ---: | ---: |
| 平均CPUフレーム時間 | 17.65 ms | **11.92 ms** | **32.5%削減** |
| CPUフレーム時間 P95 | 24.60 ms | **17.44 ms** | **29.1%削減** |
| PlayerLoop平均 | 9.03 ms | **5.46 ms** | **39.5%削減** |
| Physics.Simulate平均 | 0.49 ms | **0.17 ms** | **65.3%削減** |
| 1フレームのGC割り当て平均 | 101.0 KB | **12.0 KB** | **88.1%削減** |
| Udon平均 | **0.82 ms** | 1.08 ms | 31.7%増加 |

平均CPUフレーム時間の逆数は約56.7から83.9 FPS相当になります。これは実際に表示されたFPSではなく、CPUフレーム時間から換算した処理率です。ディスプレイのリフレッシュレート、フレーム制限、GPU負荷によって実際のFPSは変わります。

最新スナップショットでは中央シミュレーション、車線変更、車両状態の圧縮、リモート補間が追加されたため、Udon平均は初期スナップショットより高くなっています。一方でPlayerLoop、Physics、GCが大きく減り、CPUフレーム時間全体は改善しました。

`10 Hz`は走行判断の周期です。権限側とリモート側のどちらも車両Transformをレンダーフレームごとに補間するため、画面が10 FPSで更新される構成ではありません。モデル、マテリアル、レンダリング設定は下げていません。GPUフレーム時間と画質は測定していないため、この表には含めません。

## 1. 各車両が毎フレーム計算していた問題

### 問題

旧`CarNavController`は、稼働中の全車両で毎フレーム次の処理を行っていました。

- 次のWaypointまでの方向と距離を計算
- `Physics.BoxCast`で前方の車両、プレイヤー、信号、障害物を判定
- 速度と回転を計算してTransformを移動
- 車輪を回転
- `RequestSerialization()`を呼び出し

稼働車両10台、60 FPSでは、走行判断とBoxCastがそれぞれ毎秒600回実行されます。フレームレートによって移動結果も変わります。

### 適用した技術

レーンの位置と接続関係をエディタで事前生成し、`TrafficSimulationManager`一つで全車両を計算する構成へ変更しました。

- 走行判断を0.1秒の固定間隔で実行
- 長いフレームの後も1フレームの補正計算を最大4回に制限
- 前方車両と信号は物理判定ではなくレーン上の進行距離で計算
- プレイヤーと固定障害物の物理判定は所有者だけが実行
- 稼働車両を順番に、1フレーム2台ずつセンサー更新

### 数値の変化

| 計算 | 変更前 | 現在 | 変化 |
| --- | ---: | ---: | ---: |
| 車両1台の走行判断 | 60回/秒 | 10回/秒 | **83.3%削減** |
| 車両10台の走行判断 | 600回/秒 | 100回/秒 | **毎秒500回削減** |
| 1フレームの前方センサー対象 | 10台 | 2台 | **80%削減** |
| 車両10台のセンサー一巡 | 毎フレーム | 5フレームに1回 | 1フレームへの集中を回避 |

現在のセンサーは車両1台の更新で`CheckBox`と`BoxCast`を使う場合があり、車線変更時には追加判定も行います。そのため80%は物理API全体の呼び出し数ではなく、**1フレームでセンサーを更新する車両数**の削減率です。

## 2. 毎フレーム送信を要求していた問題

### 問題

旧コードでは車両10台がそれぞれ毎フレーム`RequestSerialization()`を呼び出していました。`CarSpawner`と信号機も同じ方法でした。

60 FPSで計算すると次の通りです。

- 車両10台: 毎秒600回
- スポナー: 毎秒60回
- 信号機: 毎秒60回
- 合計: 毎秒720回

これはVRChatが毎秒720パケットを送信したという意味ではありません。ネットワーク側で制限される前に、コードが送信を要求した回数です。車両ごとの状態とTransform同期も別々に存在し、送信タイミングと所有権が複数のオブジェクトに分散していました。

### 適用した技術

車両のワールド座標と回転を送る代わりに、全員が共有するレーン上の状態を送ります。

```mermaid
flowchart LR
    A[所有者が10台を計算] --> B[レーンと進行距離へ変換]
    B --> C[車両ごとに64ビットへ圧縮]
    C --> D[0.25秒ごとに一つのスナップショットを送信]
    D --> E[受信側が同じレーン上へ復元]
```

- 所有者一人だけがシミュレーションを実行
- 二つの`int`配列へ最大16台の状態を保存
- 車両1台の64ビットに稼働状態、レーン、進行距離、速度、加速度、車線変更状態を記録
- 連番と所有権の世代番号で古い状態を除外
- 直列化中またはネットワーク混雑時は新しい要求を積まない
- 信号機は毎フレームの時刻ではなく、周期を開始したサーバー時刻を共有

### 数値の変化

交通管理スクリプトは0.25秒ごとにスナップショットを作り、通常は毎秒4回の直列化を要求します。信号機は初期化時とマスター変更時だけ送信します。

| 項目 | 変更前 | 現在 |
| --- | ---: | ---: |
| 通常実行中の直列化要求 | 720回/秒 | 4回/秒 |
| 要求回数の削減 |  | **99.4%** |
| 車両状態の生データ | 比較不可 | 16 × 8バイト = **128バイト** |
| 共通値を含む生データ | 比較不可 | 128 + 12 = **140バイト** |

140バイトはC#の基本型サイズから算出した値です。Udonの直列化情報を含む実際の転送量は、実行中に`OnPostSerialization()`の`byteCount`を記録します。旧版のパケット記録が残っていないため、転送バイト数の削減率は記載していません。

実装は[`TrafficSimulationManager.cs`](../Assets/Shinjuku%20Udon/Traffic/TrafficSimulationManager.cs)と[`ShinhoTime.cs`](../Assets/Shinjuku%20Udon/Traffic/Shinho/ShinhoTime.cs)で確認できます。

## 3. 低い送信頻度でリモート車両が途切れて見える問題

### 問題

送信を毎秒4回まで下げても、受信した状態をそのまま適用すると、リモート車両は0.25秒ごとに位置が切り替わって見えます。パケットが遅れた場合は停止と移動を繰り返します。

### 適用した技術

- 直近二つのスナップショットを保持して補間
- パケットの到着間隔に合わせ、表示遅延を0.35～1.25秒の範囲で調整
- 最新スナップショットを越えた場合だけ最大0.15秒先まで予測
- レーン進行距離から位置と回転を一緒に復元
- 車線変更、緊急回避、後退復帰は既存64ビット内のビットを再利用

リモート表示を滑らかにするための同期フィールドは追加していません。各種走行機能を追加した後も、車両1台の状態は64ビットを維持しています。

## 4. 一定間隔でフレーム時間が跳ね上がる問題

### 問題

初期システムでは車両ごとに計算と物理判定を個別に実行していたため、プレイヤーと車両が増えると遅いフレームが繰り返し発生しました。Profiler Timelineでは、全車両のセンサー判定が同じフレームに集中する区間も確認できました。

### 適用した技術

信号停止はベイク済み停止線の計算へ分離し、車線変更の感知範囲はルールごとに事前計算しました。プレイヤーの物理判定は所有者だけが実行し、車両センサーを1フレーム2台ずつ更新します。車両10台の一巡を5フレームに分散し、1フレームへの集中を防いでいます。

### 結果

初期スナップショットと最新スナップショットを比較すると、CPUフレーム時間のP95は`24.60 ms → 17.44 ms`となり、29.1%減少しました。平均CPUフレーム時間も32.5%減少し、通常時の性能と繰り返し発生する遅いフレームの両方が改善しています。テスト中に記録された最大の単一フレームはClientSim自体の処理によるものだったため、交通システムの結果には使用していません。

## 5. 実在する場所の形を残すモデル最適化

### 問題

建物と看板を一つの大きなメッシュへまとめるとオブジェクト数は減りますが、建物や路地の奥に隠れた部分まで一緒に描画されやすくなります。反対に細かく分けすぎると、レンダラーとマテリアルの呼び出しが増えます。

### 適用した技術と現在の数値

| 項目 | 現在の設定 |
| --- | ---: |
| 環境モデル | 246,921三角形、240メッシュ |
| マテリアル | 77個、レンダラーマテリアルスロット313個 |
| スタティックバッチング対象 | 392オブジェクト |
| オクルーダー | 330オブジェクト |
| ベイク済みオクルージョンデータ | 3.28 MB |
| Bakery設定メッシュ | 約220 |
| ライトマップ | 4096を3枚 + 512を1枚 |
| 環境用Mesh Collider | 2個 |
| ブレンドシェイプ | 4メッシュに41個、2,624三角形 |

- 建物と街路を区域ごとに分割してオクルージョンカリングを適用
- 動かないオブジェクトにスタティックバッチングを使用
- ネオンと建物の照明をBakeryでベイク
- 不要なレンダラー264個で影の受信を無効化
- 静的レンダラー247個でモーションベクターを無効化
- Mesh Read/Writeを無効化し、頂点結合とライトマップUV生成を有効化
- モデル全体の自動コライダーを無効化し、移動に必要な2メッシュだけを使用

最適化前のモデル設定は残っていません。そのため、この項目では削減率ではなく現在のプロジェクトで確認した値だけを記載しています。

## 関連する制作ツール

| ツール | 解決した問題 | コード |
| --- | --- | --- |
| レーンデータの事前生成 | 実行中のレーン探索を減らし、切れた接続をビルド前に確認 | [`TrafficLaneBakerEditor.cs`](../Assets/Shinjuku%20Udon/Traffic/Editor/TrafficLaneBakerEditor.cs) |
| 車両・センサーの表示 | センサー範囲、現在レーン、目標レーン、ネットワーク状態をScene Viewで確認 | [`TrafficSimulationManagerEditor.cs`](../Assets/Shinjuku%20Udon/Traffic/Editor/TrafficSimulationManagerEditor.cs) |
| 80人負荷テスト | 一定間隔のフレームドロップを同じ配置で再現し、測定区間を記録 | [`TrafficPlayerStressTestEditor.cs`](../Assets/Editor/TrafficPlayerStressTestEditor.cs) |

## 未測定の値

PCとQuestの平均FPS、メインスレッド時間、実際のネットワークB/sは、同じビルドとカメラ位置で再測定する必要があります。測定データが残っていない値を推定値で埋めてはいません。

モデル、Scene、テクスチャ、ライトマップ、Profilerデータはこのリポジトリに含みません。公開コードから適用方法を確認できます。

[READMEへ戻る](../README.ja.md)
