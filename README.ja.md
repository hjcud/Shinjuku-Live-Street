...
背景を作ること以上に、誰もが気軽にライブを始め、同じ瞬間を一緒に体験できるための仕組みに力を入れました。
...
レーン配列を手入力し、実行するたびに問題を探す方法は、ワールドが大きくなるほど維持が難しくなります。そのため、Scene上のレーンをランタイム用データへ変換するベイカーと、車両、センサー、ネットワーク状態をScene Viewで確認するデバッグツールも制作しました。
