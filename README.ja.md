...
新宿ライブストリートは、ネオンが灯る新宿の街中で出演者と観客が自然に出会う、**路上ライブを中心としたVRChatソーシャルワールド**です。
...
ライブ機材の同期と交通シミュレーションを再設計しました。多人数環境で共有状態を安定して保ちながら、繰り返し発生していたCPU・物理・GCの負荷を削減しています。

### ライブ機材の同期 — 所有権と状態を一つの流れで管理

スピーカーの返却後や利用者が離れたあとに一部機能の状態が残り、途中参加したユーザーには設置済みのスピーカーが見えない問題がありました。

<p align="center">
  <img src="./Docs/images/live-performance-sync.ja.svg" alt="設置条件の確認から所有権設定、途中参加者との同期、返却時の初期化までの流れ" width="900">
</p>

設置条件の確認、所有権の設定、状態の同期、返却時の初期化を一つの流れにまとめ、全員が共有する状態と個人設定を分けて管理しました。
...
<p align="center">
  <img src="./Docs/images/traffic-system-architecture.ja.svg" alt="車両ごとの毎フレーム処理を中央シミュレーション、分散センサー、圧縮スナップショットへ変更した比較" width="900">
</p>

ベイク済みのレーンデータを基準に車両を中央で計算し、リモート側は車両1台あたり64ビットに圧縮したスナップショットを同じレーン上へ復元する構成に変更しました。描画時は状態の間を毎フレーム補間し、動きの連続性を保ちます。
...
走行判断は`10 Hz`で処理し、描画時は状態の間を毎フレーム補間して動きの連続性を保ちました。
...
環境モデルを区域ごとに分け、カメラに映らない範囲をオクルージョンカリングで除外できる構成にしました。スタティックバッチとベイク照明を組み合わせ、新宿の夜景が持つ光の密度と奥行きを保っています。
